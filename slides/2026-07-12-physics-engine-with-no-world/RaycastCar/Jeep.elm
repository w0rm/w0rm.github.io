module RaycastCar.Jeep exposing (Jeep, load, loadWith, settings, wheels)
...
type alias Jeep =
    ...
    , colliderMesh : Scene3d.Mesh.Uniform BodyCoordinates
...
{-| Like [`load`](#load) but with a custom uniform scale applied to the model
(the default `load` uses `0.5`). Useful for placing the jeep among differently
scaled props.
-}
loadWith : Float -> { texture : String, mesh : String } -> Task String Jeep
loadWith scale urls =
    Http.task
        { method = "get"
        , headers = []
                ...
                (\resp ->
                    case resp of
                        Http.GoodStatus_ _ str ->
                            Obj.Decode.decodeString (\a -> Length.meters (a * scale)) jeepDecoder str
            ...
            let
                ...
                colliderMesh =
                    Scene3d.Mesh.indexedFacets
                        (TriangularMesh.combine [ convexBase, convexWindow ])
            in
            \material ->
                { collider =
