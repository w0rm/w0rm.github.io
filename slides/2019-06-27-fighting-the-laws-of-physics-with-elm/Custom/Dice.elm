...
view { screenWidth, screenHeight, world, texture } =
    WebGL.toHtml
        ...
        (let
         ...
         in
         List.map
            (toEntity camera perspective texture)
...
toEntity : Mat4 -> Mat4 -> Maybe Texture -> Body Shape -> Entity
toEntity camera perspective maybeTexture body =
...
vertex : Shader Attributes (Uniforms a) Varying
vertex =
    [glsl|
...
fragment : Shader {} (Uniforms a) Varying
fragment =
    [glsl|
        precision mediump float;
        varying vec3 vTexturePosition;
        ...
        void main () {
          if (vTextureNumber < 6.0) {
            // Cube face
            gl_FragColor = vec4(vlighting * vec3(1.0,1.0,1.0), 1.0);
          } else {
            // Floor
...
texturedFragment : Shader {} TexturedUniforms Varying
texturedFragment =
    [glsl|
        precision mediump float;
        ...
        void main () {
          if (vTextureNumber < 6.0) {
            // Cube face
            vec3 color = vlighting * vec3(
                texture2D(
                    texture,
                    vec2(
                        (vTexturePosition.x + vTextureNumber) / 8.0, vTexturePosition.y
                    )
                )
            );
