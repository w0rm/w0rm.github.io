...
Elm.Astar.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var closeNode = F2(function (current,state) {
      return _U.update(state,
      {openNodes: A2($List.filter,F2(function (x,y) {    return !_U.eq(x,y);})(current),state.openNodes)
      ,closedNodes: A2($List._op["::"],current,state.closedNodes)});
   });
   var nodeState = F2(function (node,_p0) {    var _p1 = _p0;return A2($Dict.get,node,_p1.nodeStates);});
   var unwindParents = F2(function (node,state) {
      var _p2 = A2($Maybe.andThen,A2(nodeState,node,state),function (_) {    return _.parent;});
      if (_p2.ctor === "Nothing") {
            return _U.list([node]);
         } else {
            return A2($List._op["::"],node,A2(unwindParents,_p2._0,state));
         }
   });
   var getPath = function (state) {    return $List.reverse(A2(unwindParents,state.dest,state));};
   var distance = F2(function (_p4,_p3) {
      var _p5 = _p4;
      var _p10 = _p5._1;
      var _p9 = _p5._0;
      var _p6 = _p3;
   ...
   var processNeighbors = F3(function (current,neighbors,state) {
      processNeighbors: while (true) {
         var _p11 = neighbors;
         if (_p11.ctor === "[]") {
               return state;
            } else {
               var _p15 = _p11._1;
               var _p14 = _p11._0;
               var _p12 = A2(nodeState,_p14,state);
               if (_p12.ctor === "Nothing") {
                     var _v6 = current,_v7 = _p15,_v8 = state;
                     current = _v6;
                     neighbors = _v7;
                     state = _v8;
                     ...
                     var _p13 = _p12._0;
                     var neighborOpen = A2($List.member,_p14,state.openNodes);
                     var h = neighborOpen ? _p13.h : A2(distance,_p14,state.dest);
                     var g = current.g + A2(distance,current.node,_p14);
                     var best = $Basics.not(neighborOpen) || _U.cmp(g,_p13.g) < 0;
                     var parent = best ? $Maybe.Just(current.node) : _p13.parent;
                     var f = best ? g + h : _p13.f;
                     var nupdate = _U.update(_p13,{parent: parent,f: f,g: g,h: h});
                     var nextState = _U.update(state,
                     {nodeStates: A3($Dict.insert,_p14,nupdate,state.nodeStates)
                     ,openNodes: neighborOpen ? state.openNodes : A2($List._op["::"],_p14,state.openNodes)});
                     var _v9 = current,_v10 = _p15,_v11 = nextState;
                     current = _v9;
                     neighbors = _v10;
                     state = _v11;
                     continue processNeighbors;
   ...
   var moveTile = F2(function (_p17,_p16) {    var _p18 = _p17;var _p19 = _p16;return {ctor: "_Tuple2",_0: _p18._0 + _p19._0,_1: _p18._1 + _p19._1};});
   var onGrid = F2(function (_p21,_p20) {
      var _p22 = _p21;
      var _p23 = _p20;
      var _p25 = _p23._1;
      var _p24 = _p23._0;
      return _U.cmp(_p24,0) > -1 && (_U.cmp(_p25,0) > -1 && (_U.cmp(_p24,_p22._0) < 0 && _U.cmp(_p25,_p22._1) < 0));
   });
   var freeUnclosedNeighbors = F2(function (_p26,state) {
      var _p27 = _p26;
      var _p29 = _p27._1;
      var _p28 = _p27._0;
      return A2($List.filter,
      function (n) {
         return $Basics.not(A2($List.member,n,state.closedNodes));
      },
      A2($List.filter,
      onGrid(state.gridSize),
      A2($List.filter,
      F2(function (x,y) {    return !_U.eq(x,y);})({ctor: "_Tuple2",_0: _p28,_1: _p29}),
      A3($List.foldl,
      function (x) {
         return F2(function (x,y) {    return A2($Basics._op["++"],x,y);})(A2($List.map,
         F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};})(x),
         _U.range(_p29 - 1,_p29 + 1)));
      },
      _U.list([]),
      _U.range(_p28 - 1,_p28 + 1)))));
   });
   var initialState = F4(function (gridSize,obstacles,start,destination) {
      return {nodeStates: $Dict.empty
             ,openNodes: A2($List.all,onGrid(gridSize),_U.list([start,destination])) ? _U.list([start]) : _U.list([])
             ,closedNodes: obstacles
             ,gridSize: gridSize
             ,dest: destination};
   });
   var initialNodeState = function (node) {    return {node: node,f: 0,g: 0,h: 0,parent: $Maybe.Nothing};};
   var checkInitNodeState = F2(function (node,state) {
      if (A2(onGrid,state.gridSize,node)) {
            var _p30 = A2(nodeState,node,state);
            if (_p30.ctor === "Nothing") {
                  return _U.update(state,{nodeStates: A3($Dict.insert,node,initialNodeState(node),state.nodeStates)});
               } else {
                  return state;
               }
         } else return state;
   });
   var processNodeNeighbors = F2(function (nodeState,state) {
      var neighbors = A2(freeUnclosedNeighbors,nodeState.node,state);
      var nextState = A3($List.foldl,checkInitNodeState,state,neighbors);
      return A3(processNeighbors,nodeState,neighbors,nextState);
   });
   var astarIter = function (state) {
      astarIter: while (true) {
         var openNodeStates = A2($List.filterMap,A2($Basics.flip,nodeState,state),state.openNodes);
         var _p31 = $List.head(A2($List.sortBy,function (_) {    return _.f;},openNodeStates));
         if (_p31.ctor === "Nothing") {
               return state;
            } else {
               var _p32 = _p31._0;
               if (_U.eq(_p32.node,state.dest)) return state; else {
                     var _v19 = A2(processNodeNeighbors,_p32,A2(closeNode,_p32.node,state));
                     state = _v19;
                     continue astarIter;
                  }
            }
   ...
   var findPath = F4(function (gridSize,obstacles,start,destination) {
      return astarIter(A3($Basics.flip,$List.foldl(checkInitNodeState),_U.list([start,destination]),A4(initialState,gridSize,obstacles,start,destination)));
   });
   var State = F5(function (a,b,c,d,e) {    return {nodeStates: a,openNodes: b,closedNodes: c,gridSize: d,dest: e};});
   var NodeState = F5(function (a,b,c,d,e) {    return {node: a,f: b,g: c,h: d,parent: e};});
   return _elm.Astar.values = {_op: _op,findPath: findPath,getPath: getPath};
};
Elm.Fountain = Elm.Fountain || {};
Elm.Fountain.make = function (_elm) {
   "use strict";
   _elm.Fountain = _elm.Fountain || {};
   if (_elm.Fountain.values) return _elm.Fountain.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   $Time = Elm.Time.make(_elm);
   var _op = {};
   var animationLoop = F2(function (elapsed,state) {
      var elapsed$ = state.elapsed + elapsed;
      return _U.cmp(elapsed$,state.timeout) > -1 ? _U.update(state,
      {elapsed: elapsed$ - state.timeout,frame: A2($Basics._op["%"],state.frame + 1,4)}) : _U.update(state,{elapsed: elapsed$});
   });
   var animate = F2(function (time,model) {
      var _p0 = model.prevTime;
      if (_p0.ctor === "Nothing") {
            return _U.update(model,{prevTime: $Maybe.Just(time)});
         } else {
            return A2(animationLoop,A2($Basics.min,time - _p0._0,25),_U.update(model,{prevTime: $Maybe.Just(time)}));
         }
   });
   var Model = F5(function (a,b,c,d,e) {    return {prevTime: a,state: b,frame: c,elapsed: d,timeout: e};});
   var Stopped = {ctor: "Stopped"};
   var initial = {state: Stopped,prevTime: $Maybe.Nothing,elapsed: 0,timeout: 1000,frame: 0};
   var Playing = {ctor: "Playing"};
   var Tick = function (a) {    return {ctor: "Tick",_0: a};};
   var update = F2(function (action,model) {
      var _p1 = action;
      if (_p1.ctor === "Click") {
            return _U.eq(model.state,Playing) ? {ctor: "_Tuple2",_0: _U.update(model,{state: Stopped}),_1: $Effects.none} : {ctor: "_Tuple2"
                                                                                                                            ,_0: _U.update(model,
                                                                                                                            {state: Playing})
                                                                                                                            ,_1: $Effects.tick(Tick)};
         } else {
            return _U.eq(model.state,Playing) ? {ctor: "_Tuple2",_0: A2(animate,_p1._0,model),_1: $Effects.tick(Tick)} : {ctor: "_Tuple2"
                                                                                                                         ,_0: _U.update(model,
                                                                                                                         {prevTime: $Maybe.Nothing})
                                                                                                                         ,_1: $Effects.none};
         }
   });
   ...
   var view = F2(function (address,_p2) {
      ...
      _U.list([A2($Html.div,
              _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "float",_1: "left"}
                                                      ...
                                                      ,{ctor: "_Tuple2",_0: "padding",_1: "30px 0 0 0"}]))]),
              _U.list([A2($Html.div,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],"state = ",_U.eq(_p3.state,Playing) ? "Playing" : "Stopped"))]))
                      ,A2($Html.div,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],"timeout = ",$Basics.toString(_p3.timeout)))]))
                      ,A2($Html.div,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],"elapsed = ",$Basics.toString($Basics.round(_p3.elapsed))))]))
                      ,A2($Html.div,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],"frame = ",$Basics.toString(_p4)))]))]))
              ,A2($Html.div,
              _U.list([A2($Html$Events.onClick,address,Click)
                      ,$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "position",_1: "relative"}
              ...
              _U.list([A2($Html.div,
              _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "width",_1: "80px"}
                                                      ,{ctor: "_Tuple2",_0: "height",_1: "160px"}
                                                      ...
                                                      ,{ctor: "_Tuple2"
                                                       ...
                                                       ,_1: A2($Basics._op["++"],$Basics.toString((0 - _p4) * 80),"px 0")}]))]),
              _U.list([]))]))
              ,A2($Html.div,_U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "clear",_1: "both"}]))]),_U.list([]))]));
   });
   return _elm.Fountain.values = {_op: _op,initial: initial,update: update,view: view,Model: Model};
};
Elm.Pathfinder = Elm.Pathfinder || {};
Elm.Pathfinder.make = function (_elm) {
   "use strict";
   _elm.Pathfinder = _elm.Pathfinder || {};
   if (_elm.Pathfinder.values) return _elm.Pathfinder.values;
   var _U = Elm.Native.Utils.make(_elm),
   ...
   var _op = {};
   var addPointToSring = F2(function (tileSize,_p0) {
      var _p1 = _p0;
      return F2(function (x,y) {    return A2($Basics._op["++"],x,y);})(A2($Basics._op["++"],
      " ",
      A2($Basics._op["++"],
      $Basics.toString(_p1._0 * tileSize + (tileSize / 2 | 0)),
      A2($Basics._op["++"],",",$Basics.toString(_p1._1 * tileSize + (tileSize / 2 | 0))))));
   });
   var renderPoints = F2(function (tileSize,waypoints) {
      return A2($Svg.polyline,
      _U.list([$Svg$Attributes.points(A3($List.foldl,addPointToSring(tileSize),"",waypoints))
   ...
   var renderPath = F3(function (_p2,tileSize,route) {
      var _p3 = _p2;
      var _p5 = _p3._0;
      var _p4 = _p3._1;
      return A2($Svg.svg,
      _U.list([$Svg$Attributes.version("1.1")
              ...
              "0 0 ",
              A2($Basics._op["++"],$Basics.toString(_p5 * tileSize),A2($Basics._op["++"]," ",$Basics.toString(_p4 * tileSize)))))
              ,$Svg$Attributes.width($Basics.toString(_p5 * tileSize))
              ,$Svg$Attributes.height($Basics.toString(_p4 * tileSize))
              ,$Svg$Attributes.style("position: absolute")]),
      _U.list([A2(renderPoints,tileSize,route)]));
   });
   var renderSquare = F2(function (color,_p6) {
      var _p7 = _p6;
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "left",_1: A2($Basics._op["++"],$Basics.toString(_p7._0 * 40),"px")}
                                              ,{ctor: "_Tuple2",_0: "top",_1: A2($Basics._op["++"],$Basics.toString(_p7._1 * 40),"px")}
   ...
   var renderItem = F2(function (name,_p8) {
      var _p9 = _p8;
      return A2($Html.img,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "left",_1: A2($Basics._op["++"],$Basics.toString(_p9._0 * 40),"px")}
                                              ,{ctor: "_Tuple2",_0: "top",_1: A2($Basics._op["++"],$Basics.toString(_p9._1 * 40),"px")}
                                              ,{ctor: "_Tuple2",_0: "width",_1: "40px"}
                                              ,{ctor: "_Tuple2",_0: "height",_1: "40px"}
                                              ,{ctor: "_Tuple2",_0: "position",_1: "absolute"}]))
              ,$Html$Attributes.src(A2($Basics._op["++"],"assets/",A2($Basics._op["++"],name,".png")))]),
      _U.list([]));
   });
   var update = F2(function (action,model) {
      var _p10 = action;
      return {ctor: "_Tuple2",_0: _U.update(model,{dest: $Maybe.Just(_p10._0)}),_1: $Effects.none};
   });
   var tiles = function (_p11) {
      var _p12 = _p11;
      return A3($List.foldl,
      function (x) {
         return F2(function (x,y) {    return A2($Basics._op["++"],x,y);})(A2($List.map,
         F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};})(x),
         _U.range(0,_p12._1 - 1)));
      },
      _U.list([]),
      _U.range(0,_p12._0 - 1));
   };
   var initial = {source: {ctor: "_Tuple2",_0: 5,_1: 5}
                 ,dest: $Maybe.Nothing
                 ,obstacles: _U.list([{ctor: "_Tuple2",_0: 3,_1: 5}
   ...
   var renderTile = F3(function (address,color,_p13) {
      var _p14 = _p13;
      var _p16 = _p14._1;
      var _p15 = _p14._0;
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "left",_1: A2($Basics._op["++"],$Basics.toString(_p15 * 40),"px")}
                                              ,{ctor: "_Tuple2",_0: "top",_1: A2($Basics._op["++"],$Basics.toString(_p16 * 40),"px")}
                                              ,{ctor: "_Tuple2",_0: "width",_1: "40px"}
                                              ,{ctor: "_Tuple2",_0: "height",_1: "40px"}
                                              ,{ctor: "_Tuple2",_0: "position",_1: "absolute"}
                                              ,{ctor: "_Tuple2",_0: "background",_1: color}]))
              ,A2($Html$Events.onClick,address,Click({ctor: "_Tuple2",_0: _p15,_1: _p16}))]),
      _U.list([]));
   });
   var view = F2(function (address,_p17) {
      var _p18 = _p17;
      ...
      var _p19 = function () {
         ...
         if (_p20.ctor === "Nothing") {
               ...
               var state = A4($Astar.findPath,_p23,_p22,_p24,_p20._0);
               var path = $Astar.getPath(state);
               var _p21 = state;
      ...
      var closed = _p19._1;
      var route = _p19._2;
      var destinations = A2($List.filter,
      F2(function (x,y) {    return !_U.eq(x,y);})(_p24),
      A2($List.filter,function (a) {    return $Basics.not(A2($List.member,a,_p22));},tiles(_p23)));
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "width",_1: A2($Basics._op["++"],$Basics.toString($Basics.fst(_p23) * 40),"px")}
                                              ,{ctor: "_Tuple2",_0: "height",_1: A2($Basics._op["++"],$Basics.toString($Basics.snd(_p23) * 40),"px")}
                                              ,{ctor: "_Tuple2",_0: "position",_1: "relative"}
                                              ,{ctor: "_Tuple2"
                                               ,_0: "background"
                                               ,_1: "linear-gradient(0deg, #ccc 0, rgba(0,0,0,0) 1px, rgba(0,0,0,0) 100%), linear-gradient(90deg, #ccc 0, rgba(0,0,0,0) 1px, rgba(0,0,0,0) 100%)"}
      ...
      $List.concat(_U.list([A2($List.map,renderSquare("#eee"),opened)
                           ,A2($List.map,renderSquare("#ddd"),closed)
                           ,A2($List.map,renderItem("house"),_p22)
                           ,_U.list([A2(renderItem,"delivery-person",_p24),A3(renderPath,_p23,40,route)])
                           ,A2($List.map,A2(renderTile,address,"transparent"),destinations)])));
   });
   var Model = F4(function (a,b,c,d) {    return {source: a,dest: b,obstacles: c,size: d};});
   return _elm.Pathfinder.values = {_op: _op,view: view,initial: initial,update: update,Model: Model};
};
Elm.Randomize = Elm.Randomize || {};
Elm.Randomize.make = function (_elm) {
   "use strict";
   _elm.Randomize = _elm.Randomize || {};
   if (_elm.Randomize.values) return _elm.Randomize.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Astar = Elm.Astar.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var size = function (_p0) {    var _p1 = _p0;return _p1.size;};
   var sortBySize = function (_p2) {
      return $List.reverse(A2($List.sortBy,function (_p3) {    var _p4 = _p3;var _p5 = _p4.size;return $Basics.fst(_p5) * $Basics.snd(_p5);},_p2));
   };
   var filterSize = function (_p6) {
      var _p7 = _p6;
      return $List.filter(function (_p8) {
         var _p9 = _p8;
         var _p10 = _p9.size;
         return _U.cmp($Basics.fst(_p10),_p7._0) > -1 && _U.cmp($Basics.snd(_p10),_p7._1) > -1;
      });
   };
   var fitRandom = F2(function (_p12,_p11) {
      var _p13 = _p12;
      var _p16 = _p13.size;
      var _p15 = _p13.position;
      var _p14 = _p11;
      return A2($Random.pair,
      A2($Random.$int,$Basics.fst(_p15),$Basics.fst(_p15) + $Basics.fst(_p16) - _p14._0),
      A2($Random.$int,$Basics.snd(_p15),$Basics.snd(_p15) + $Basics.snd(_p16) - _p14._1));
   });
   var splitBy = F2(function (box1,box2) {
      var _p17 = box2.size;
      var w2 = _p17._0;
      var h2 = _p17._1;
      var _p18 = box2.position;
      var x2 = _p18._0;
      var y2 = _p18._1;
      var _p19 = box1.size;
      var w1 = _p19._0;
      var h1 = _p19._1;
      var _p20 = box1.position;
      var x1 = _p20._0;
      var y1 = _p20._1;
      return A2($List.filter,
      function (_p21) {
         var _p22 = _p21;
         var _p23 = _p22.size;
         return _U.cmp($Basics.fst(_p23),0) > 0 && _U.cmp($Basics.snd(_p23),0) > 0;
      },
      _U.list([{position: {ctor: "_Tuple2",_0: x2,_1: y2},size: {ctor: "_Tuple2",_0: x1 - x2,_1: h1 + y1 - y2}}
              ,{position: {ctor: "_Tuple2",_0: x1,_1: y2},size: {ctor: "_Tuple2",_0: x2 + w2 - x1,_1: y1 - y2}}
              ,{position: {ctor: "_Tuple2",_0: x1 + w1,_1: y1},size: {ctor: "_Tuple2",_0: x2 + w2 - (x1 + w1),_1: y2 + h2 - y1}}
              ,{position: {ctor: "_Tuple2",_0: x2,_1: y1 + h1},size: {ctor: "_Tuple2",_0: x1 + w1 - x2,_1: y2 + h2 - (y1 + h1)}}]));
   });
   var placeRandom = F2(function (object,boxes) {
      var _p24 = A2(filterSize,size(object),boxes);
      if (_p24.ctor === "[]") {
            return A2($Random.map,$Basics.always({ctor: "_Tuple2",_0: $Maybe.Nothing,_1: _U.list([])}),A2($Random.$int,0,0));
         } else {
            var _p25 = _p24._0;
            return A2($Random.map,
            function (position) {
               return {ctor: "_Tuple2"
                      ,_0: $Maybe.Just(_U.update(object,{position: position}))
                      ,_1: sortBySize(A2($Basics._op["++"],A2(splitBy,{position: position,size: size(object)},_p25),_p24._1))};
            },
            A2(fitRandom,_p25,size(object)));
         }
   });
   var renderItem = function (_p26) {
      var _p27 = _p26;
      var _p29 = _p27.size;
      var _p28 = _p27.position;
      return A2($Html.img,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "left",_1: A2($Basics._op["++"],$Basics.toString($Basics.fst(_p28) * 20),"px")}
                                              ,{ctor: "_Tuple2",_0: "top",_1: A2($Basics._op["++"],$Basics.toString($Basics.snd(_p28) * 20),"px")}
                                              ,{ctor: "_Tuple2",_0: "width",_1: A2($Basics._op["++"],$Basics.toString($Basics.fst(_p29) * 20),"px")}
                                              ,{ctor: "_Tuple2",_0: "height",_1: A2($Basics._op["++"],$Basics.toString($Basics.snd(_p29) * 20),"px")}
                                              ,{ctor: "_Tuple2",_0: "position",_1: "absolute"}]))
              ,$Html$Attributes.src(A2($Basics._op["++"],
              "assets/element-",
              A2($Basics._op["++"],
              $Basics.toString($Basics.fst(_p29)),
              A2($Basics._op["++"],"x",A2($Basics._op["++"],$Basics.toString($Basics.snd(_p29)),".png")))))]),
      _U.list([]));
   };
   var renderSquare = function (_p30) {
      var _p31 = _p30;
      var _p33 = _p31.size;
      var _p32 = _p31.position;
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "left",_1: A2($Basics._op["++"],$Basics.toString($Basics.fst(_p32) * 20),"px")}
                                              ,{ctor: "_Tuple2",_0: "top",_1: A2($Basics._op["++"],$Basics.toString($Basics.snd(_p32) * 20),"px")}
                                              ,{ctor: "_Tuple2",_0: "width",_1: A2($Basics._op["++"],$Basics.toString($Basics.fst(_p33) * 20),"px")}
                                              ,{ctor: "_Tuple2",_0: "height",_1: A2($Basics._op["++"],$Basics.toString($Basics.snd(_p33) * 20),"px")}
                                              ,{ctor: "_Tuple2",_0: "position",_1: "absolute"}
                                              ,{ctor: "_Tuple2",_0: "border",_1: "1px solid black"}]))]),
      _U.list([]));
   };
   var initial = {seed: $Random.initialSeed(0)
                 ,boxes: _U.list([{size: {ctor: "_Tuple2",_0: 30,_1: 20},position: {ctor: "_Tuple2",_0: 0,_1: 0}}])
                 ,elements: _U.list([])
                 ,size: {ctor: "_Tuple2",_0: 30,_1: 20}};
   var update = F2(function (action,model) {
      var _p34 = action;
      if (_p34.ctor === "Click") {
            var _p35 = A2($Random.generate,A2(placeRandom,{position: {ctor: "_Tuple2",_0: 0,_1: 0},size: _p34._0},model.boxes),model.seed);
            var el = _p35._0._0;
            var boxes = _p35._0._1;
            var seed = _p35._1;
            var _p36 = el;
            if (_p36.ctor === "Just") {
                  return {ctor: "_Tuple2"
                         ,_0: _U.update(model,{elements: A2($List._op["::"],_p36._0,model.elements),boxes: boxes,seed: seed})
                         ,_1: $Effects.none};
               } else {
                  return {ctor: "_Tuple2",_0: model,_1: $Effects.none};
               }
         } else {
            return {ctor: "_Tuple2",_0: initial,_1: $Effects.none};
         }
   });
   var Model = F4(function (a,b,c,d) {    return {seed: a,elements: b,boxes: c,size: d};});
   var Box = F2(function (a,b) {    return {position: a,size: b};});
   var Reset = {ctor: "Reset"};
   var Click = function (a) {    return {ctor: "Click",_0: a};};
   var renderClickableItem = F2(function (address,_p37) {
      var _p38 = _p37;
      var _p40 = _p38.size;
      var _p39 = _p38.position;
      return A2($Html.img,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "left",_1: A2($Basics._op["++"],$Basics.toString($Basics.fst(_p39) * 20),"px")}
                                              ,{ctor: "_Tuple2",_0: "top",_1: A2($Basics._op["++"],$Basics.toString($Basics.snd(_p39) * 20),"px")}
                                              ,{ctor: "_Tuple2",_0: "width",_1: A2($Basics._op["++"],$Basics.toString($Basics.fst(_p40) * 20),"px")}
                                              ,{ctor: "_Tuple2",_0: "height",_1: A2($Basics._op["++"],$Basics.toString($Basics.snd(_p40) * 20),"px")}
                                              ,{ctor: "_Tuple2",_0: "position",_1: "absolute"}]))
              ,A2($Html$Events.onClick,address,Click(_p40))
              ,$Html$Attributes.src(A2($Basics._op["++"],
              "assets/element-",
              A2($Basics._op["++"],
              $Basics.toString($Basics.fst(_p40)),
              A2($Basics._op["++"],"x",A2($Basics._op["++"],$Basics.toString($Basics.snd(_p40)),".png")))))]),
      _U.list([]));
   });
   var view = F2(function (address,_p41) {
      var _p42 = _p41;
      var _p43 = _p42.size;
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "width",_1: A2($Basics._op["++"],$Basics.toString($Basics.fst(_p43) * 20),"px")}
                                              ,{ctor: "_Tuple2",_0: "height",_1: A2($Basics._op["++"],$Basics.toString($Basics.snd(_p43) * 20),"px")}
                                              ,{ctor: "_Tuple2",_0: "position",_1: "relative"}
                                              ,{ctor: "_Tuple2"
                                               ,_0: "background"
                                               ,_1: "linear-gradient(0deg, #ccc 0, rgba(0,0,0,0) 1px, rgba(0,0,0,0) 100%), linear-gradient(90deg, #ccc 0, rgba(0,0,0,0) 1px, rgba(0,0,0,0) 100%)"}
      ...
      A2($List._op["::"],
      A2(renderClickableItem,address,{size: {ctor: "_Tuple2",_0: 2,_1: 3},position: {ctor: "_Tuple2",_0: $Basics.fst(_p43) + 1,_1: 0}}),
      A2($List._op["::"],
      A2(renderClickableItem,address,{size: {ctor: "_Tuple2",_0: 3,_1: 3},position: {ctor: "_Tuple2",_0: $Basics.fst(_p43) + 1,_1: 4}}),
      A2($List._op["::"],
      A2(renderClickableItem,address,{size: {ctor: "_Tuple2",_0: 3,_1: 5},position: {ctor: "_Tuple2",_0: $Basics.fst(_p43) + 1,_1: 8}}),
      $List.concat(_U.list([A2($List.map,renderItem,_p42.elements),A2($List.map,renderSquare,_p42.boxes)]))))));
   });
   return _elm.Randomize.values = {_op: _op,view: view,initial: initial,update: update,Model: Model};
};
Elm.Model = Elm.Model || {};
Elm.Model.make = function (_elm) {
   ...
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Fountain = Elm.Fountain.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Pathfinder = Elm.Pathfinder.make(_elm),
   $Randomize = Elm.Randomize.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var RandomizeModel = function (a) {    return {ctor: "RandomizeModel",_0: a};};
   var randomize = RandomizeModel($Randomize.initial);
   var PathfinderModel = function (a) {    return {ctor: "PathfinderModel",_0: a};};
   var pathfinder = PathfinderModel($Pathfinder.initial);
   var FountainModel = function (a) {    return {ctor: "FountainModel",_0: a};};
   var fountain = FountainModel($Fountain.initial);
...
Elm.Formatting = Elm.Formatting || {};
Elm.Formatting.make = function (_elm) {
   "use strict";
   _elm.Formatting = _elm.Formatting || {};
   if (_elm.Formatting.values) return _elm.Formatting.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var scale = function (ratio) {
      return $SliceShow$Content.container($Html.div(_U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2"
                                                                                             ,_0: "transform"
                                                                                             ,_1: A2($Basics._op["++"],
                                                                                             "scale(",
                                                                                             A2($Basics._op["++"],$Basics.toString(ratio),")"))}
                                                                                            ,{ctor: "_Tuple2",_0: "transform-origin",_1: "0 0"}]))])));
   };
   var group = $SliceShow$Content.container($Html.div(_U.list([])));
   var $char = function (str) {
      return $SliceShow$Content.item(A2($Html.span,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "font",_1: "30px/60px FiraSans-Regular, sans-serif"}
                                              ,{ctor: "_Tuple2",_0: "display",_1: "inline-block"}
                                              ,{ctor: "_Tuple2",_0: "vertical-align",_1: "middle"}
                                              ,{ctor: "_Tuple2",_0: "color",_1: "#666"}]))]),
      _U.list([$Html.text(str)])));
   };
   var split = F2(function (leftContent,rightContent) {
      return A2($SliceShow$Content.container,
      $Html.table(_U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "width",_1: "100%"}
                                                          ,{ctor: "_Tuple2",_0: "table-layout",_1: "fixed"}
                                                          ,{ctor: "_Tuple2",_0: "border-spacing",_1: "0"}
                                                          ,{ctor: "_Tuple2",_0: "border-collapse",_1: "collapse"}]))])),
      _U.list([A2($SliceShow$Content.container,$Html.td(_U.list([])),leftContent),A2($SliceShow$Content.container,$Html.td(_U.list([])),rightContent)]));
   });
   ...
   var demo = function (url) {
      return $SliceShow$Content.item(A2($Html.iframe,
   ...
   var position = function (_p0) {
      var _p1 = _p0;
      return $SliceShow$Content.container($Html.div(_U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "position",_1: "absolute"}
                                                                                            ,{ctor: "_Tuple2"
                                                                                             ,_0: "left"
   ...
   var spacing = function (px) {
      return $SliceShow$Content.item(A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "height",_1: A2($Basics._op["++"],$Basics.toString(px),"px")}]))]),
      _U.list([])));
   };
   ...
   var richtext = function (_p2) {    return $SliceShow$Content.item($Markdown.toHtml(_p2));};
   var image = F2(function (_p3,url) {
      var _p4 = _p3;
      return $SliceShow$Content.item(A2($Html.img,
      _U.list([$Html$Attributes.src(url)
              ,$Html$Attributes.width(_p4._0)
              ,$Html$Attributes.height(_p4._1)
              ,$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "vertical-align",_1: "middle"}]))]),
      _U.list([])));
   });
   var icon = function (name) {    return A2(image,{ctor: "_Tuple2",_0: 60,_1: 60},A2($Basics._op["++"],"assets/",A2($Basics._op["++"],name,".png")));};
   var title = function (message) {    return $SliceShow$Content.item(A2($Html.h1,_U.list([]),_U.list([$Html.text(message)])));};
   var align = function (align) {
      ...
      return $SliceShow$Content.container($Html.div(_U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "text-align",_1: alignValue}]))])));
   };
   var slidePadding = "50px 100px";
   var padded = $SliceShow$Content.container($Html.div(_U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "padding",_1: slidePadding}]))])));
   var background = function (url) {
      return $SliceShow$Content.container($Html.div(_U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "height",_1: "100%"}
                                                                                            ,{ctor: "_Tuple2",_0: "box-sizing",_1: "border-box"}
                                                                                            ,{ctor: "_Tuple2"
                                                                                             ,_0: "background-image"
                                                                                             ,_1: A2($Basics._op["++"],"url(",A2($Basics._op["++"],url,")"))}
                                                                                            ,{ctor: "_Tuple2",_0: "background-size",_1: "cover"}
                                                                                            ,{ctor: "_Tuple2",_0: "padding",_1: slidePadding}]))])));
   };
   ...
   var Left = {ctor: "Left"};
   return _elm.Formatting.values = {_op: _op
...
Elm.Slides.make = function (_elm) {
   ...
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Formatting = Elm.Formatting.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Model = Elm.Model.make(_elm),
   $Result = Elm.Result.make(_elm),
   ...
   $SliceShow$Slide = Elm.SliceShow.Slide.make(_elm);
   var _op = {};
   var futurePlans = $Formatting.bullets(A2($List.map,
   $Formatting.bullet,
   _U.list(["Fix touch support","Complete the visuals","Add sound effects","Improve the gameplay"])));
   var transitionToWebGL = A2($Formatting.split,
   _U.list([$Formatting.title("elm-html")
           ,$Formatting.bullets(_U.list([$Formatting.bullet("render happens in view")
                                        ,$Formatting.bullet("game objects become boxes")
                                        ,$Formatting.bullet("some boxes have onClick event")
                                        ,$Formatting.bullet("boxes are sorted by y-coordinate")
                                        ,$Formatting.bullet("boxes become div tags with z-index for layer")]))
           ,$Formatting.spacing(40)
           ,$SliceShow$Content.hide($Formatting.bullets(_U.list([$Formatting.bullet("slow diff of virtual dom")
                                                                ,$Formatting.bullet("slow updates of the real dom")
                                                                ,$Formatting.bullet("pass Signal.Address around")])))]),
   _U.list([$SliceShow$Content.hide($Formatting.group(_U.list([$Formatting.title("elm-webgl")
                                                              ,$Formatting.bullets(_U.list([$Formatting.bullet("render happens in update")
                                                                                           ,$Formatting.bullet("game objects become clickable and textured boxes")
                                                                                           ,$Formatting.bullet("boxes are sorted by layer and y-coordinate")
                                                                                           ,$Formatting.bullet("clickable boxes are used in action from the touch input signal")
                                                                                           ,$Formatting.bullet("textured boxes are rendered with WebGL")]))])))]));
   var gameLoop = A2($Formatting.scale,
   0.8,
   _U.list([A2($Formatting.code,
   "elm",
   ...
   var animationExample = _U.list([$Formatting.title("Animation: example")
                                  ,A2($Formatting.scale,
                                  0.9,
                                  _U.list([A2($Formatting.code,
                                  "elm",
                                  "animationLoop elapsed model =\n  let\n    elapsed\' = model.elapsed + elapsed\n  in\n    if elapsed\' >= model.timeout then\n      { model\n      | elapsed = elapsed\' - model.timeout\n      , frame = (model.frame + 1) % 4\n      }\n    else\n      { model\n      | elapsed = elapsed\'\n      }")]))
                                  ,$SliceShow$Content.hide(A2($Formatting.position,
                                  {ctor: "_Tuple2",_0: 500,_1: 400},
                                  _U.list([$SliceShow$Content.custom($Model.fountain)])))]);
   var animation = _U.list([$Formatting.title("Animation")
                           ,A2($Formatting.code,
                           "elm",
   ...
   var refactoring = _U.list([A2($Formatting.split,
                             _U.list([$Formatting.$char("[")
                                     ,$Formatting.icon("warehouse")
                                     ,$Formatting.$char("] ")
                                     ,$Formatting.$char("[")
                                     ,$Formatting.icon("house")
                                     ,$Formatting.$char("] ")
                                     ,$Formatting.$char("[(")
                                     ,$Formatting.icon("fountain")
                                     ,$Formatting.icon("tree")
                                     ,$Formatting.$char(")]")]),
                             _U.list([A2($Formatting.code,"elm","{ a | position : (Float, Float)\n    , size : (Float, Float)\n    }")]))
                             ,$SliceShow$Content.hide(A2($Formatting.split,
                             _U.list([$Formatting.$char("[(")
                                     ,$Formatting.icon("warehouse")
                                     ,$Formatting.icon("house")
                                     ,$Formatting.icon("fountain")
                                     ,$Formatting.icon("tree")
                                     ,$Formatting.$char(")]")]),
                             _U.list([A2($Formatting.code,"elm","{ position : (Float, Float)\n, size : (Float, Float)\n, category : MapObjectCategory\n}")])))
                             ,$SliceShow$Content.hide(A2($Formatting.code,
                             "elm",
                             "type MapObjectCategory = HouseCategory Int\n                       | WarehouseCategory Int\n                       | FountainCategory Fountain\n                       | TreeCategory"))]);
   var modelExamples = A2($Formatting.split,
   _U.list([$Formatting.icon("house")
           ,$Formatting.$char(" [")
           ,$Formatting.icon("category1")
           ,$Formatting.$char("], [")
           ,$Formatting.icon("category2-return")
           ,$Formatting.$char("]")
           ,$Formatting.spacing(10)
           ,$Formatting.icon("warehouse")
           ,$Formatting.$char(" [")
           ,$Formatting.icon("category2")
           ,$Formatting.$char(", ")
           ,$Formatting.icon("category3")
           ,$Formatting.$char("]")
           ,$Formatting.spacing(10)
           ,$Formatting.icon("delivery")
           ,$Formatting.$char(" [")
           ,$Formatting.icon("category3")
           ,$Formatting.$char(", ")
           ,$Formatting.icon("category1")
           ,$Formatting.$char("]")
           ,$Formatting.spacing(40)
           ,$Formatting.bullets(A2($List.map,$Formatting.bullet,_U.list(["click an article","pickup an article","spawn a new order"])))]),
   _U.list([$SliceShow$Content.hide($Formatting.group(_U.list([$Formatting.$char("[ ")
                                                              ,$Formatting.icon("category1")
                                                              ,$Formatting.$char("@")
                                                              ,$Formatting.icon("house")
                                                              ,$Formatting.$char(", ")
                                                              ...
                                                              ,$Formatting.icon("delivery")
                                                              ,$Formatting.$char(" ]")
                                                              ,$Formatting.spacing(30)
                                                              ,A2($Formatting.code,
                                                              "elm",
                                                              "type State\n  = InStock Warehouse\n  | AwaitingReturn House\n  | Delivered House\n  | Picked")])))]));
   var game = _U.list([$Formatting.demo("http://zalando.github.io/elm-street-404/")
                      ,A2($Formatting.position,{ctor: "_Tuple2",_0: 60,_1: 550},_U.list([$Formatting.richtext("[prev slide](#2)")]))
                      ,A2($Formatting.position,{ctor: "_Tuple2",_0: 860,_1: 550},_U.list([$Formatting.richtext("[next slide](#4)")]))]);
   var intro = _U.list([$Formatting.title("Creating a Fun Game With Elm")
                       ,$Formatting.spacing(60)
                       ,A2($Formatting.image,{ctor: "_Tuple2",_0: 100,_1: 137},"assets/delivery-person.png")
                       ,$Formatting.$char("       ")
                       ,A2($Formatting.image,{ctor: "_Tuple2",_0: 100,_1: 100},"assets/elm.png")
                       ,$Formatting.$char("      ")
                       ,A2($Formatting.image,{ctor: "_Tuple2",_0: 130,_1: 130},"assets/zalando.jpg")
                       ,$Formatting.spacing(60)
                       ,$Formatting.richtext("Andrey Kuzmin [@unsoundscapes](https://twitter.com/unsoundscapes)")
                       ,$SliceShow$Content.hide($Formatting.richtext("elm—([unsoundscapes](https://github.com/w0rm/elm-unsoundscapes),\n[flatris](https://github.com/w0rm/elm-flatris),\n[street-404](https://github.com/zalando/elm-street-404),\n[slice-show](https://github.com/w0rm/elm-slice-show))"))]);
   var slides = A2($List.map,
   $SliceShow$Slide.slide,
   _U.list([_U.list([$Formatting.padded(intro)])
           ,_U.list([A2($Formatting.background,"assets/team.jpg",_U.list([A2($Formatting.align,$Formatting.Right,_U.list([$Formatting.title("Idea")]))]))])
           ,game
           ,_U.list([$Formatting.shout("Model the data to reduce coplexity")])
           ,_U.list([$Formatting.padded(_U.list([$Formatting.title("Model: articles"),modelExamples]))])
           ,_U.list([$Formatting.shout("Model fast, refactor later")])
           ,_U.list([$Formatting.padded(A2($List._op["::"],$Formatting.title("Model: refactor map objects"),refactoring))])
           ,_U.list([A2($Formatting.background,"assets/refactor.png",_U.list([]))])
           ,_U.list([$Formatting.shout("Effects.tick is a ready-to-use animation loop")])
           ,_U.list([$Formatting.padded(animation)])
           ,_U.list([$Formatting.padded(animationExample)])
           ,_U.list([$Formatting.padded(_U.list([$Formatting.title("Animation: game loop"),gameLoop]))])
           ,_U.list([$Formatting.shout("elm-html is a fast rendering engine")])
           ,_U.list([$Formatting.shout("elm-webgl is a fast rendering engine for games")])
           ,_U.list([$Formatting.padded(_U.list([transitionToWebGL]))])
           ,_U.list([A2($Formatting.background,"assets/timeline.png",_U.list([]))])
           ,_U.list([$Formatting.shout("It’s fun to write purely functional algorithms")])
           ,_U.list([$Formatting.padded(_U.list([$Formatting.title("Algorithms: randomize"),$SliceShow$Content.custom($Model.randomize)]))])
           ,_U.list([$Formatting.padded(_U.list([$Formatting.title("Algorithms: pathfinder"),$SliceShow$Content.custom($Model.pathfinder)]))])
           ,_U.list([$Formatting.padded(_U.list([$Formatting.title("Future Plans"),futurePlans]))])
           ,_U.list([A2($Formatting.background,"assets/stickers.jpg",_U.list([$Formatting.title("Questions?")]))])]));
   return _elm.Slides.values = {_op: _op,slides: slides};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   var RandomizeAction = function (a) {    return {ctor: "RandomizeAction",_0: a};};
   var PathfinderAction = function (a) {    return {ctor: "PathfinderAction",_0: a};};
   var FountainAction = function (a) {    return {ctor: "FountainAction",_0: a};};
   var update = F2(function (action,model) {
      var _p0 = {ctor: "_Tuple2",_0: action,_1: model};
      _v0_3: do {
         if (_p0.ctor === "_Tuple2") {
               switch (_p0._0.ctor)
               {case "FountainAction": if (_p0._1.ctor === "FountainModel") {
                          var _p1 = A2($Fountain.update,_p0._0._0,_p0._1._0);
                          var newModel = _p1._0;
                          var newEffect = _p1._1;
                          return {ctor: "_Tuple2",_0: $Model.FountainModel(newModel),_1: A2($Effects.map,FountainAction,newEffect)};
                       } else {
                          break _v0_3;
                       }
                  case "PathfinderAction": if (_p0._1.ctor === "PathfinderModel") {
                          var _p2 = A2($Pathfinder.update,_p0._0._0,_p0._1._0);
                          var newModel = _p2._0;
                          var newEffect = _p2._1;
                          return {ctor: "_Tuple2",_0: $Model.PathfinderModel(newModel),_1: A2($Effects.map,PathfinderAction,newEffect)};
                       } else {
                          break _v0_3;
                       }
                  default: if (_p0._1.ctor === "RandomizeModel") {
                          var _p3 = A2($Randomize.update,_p0._0._0,_p0._1._0);
                          var newModel = _p3._0;
                          var newEffect = _p3._1;
                          return {ctor: "_Tuple2",_0: $Model.RandomizeModel(newModel),_1: A2($Effects.map,RandomizeAction,newEffect)};
                       } else {
                          break _v0_3;
                       }}
            } else {
   ...
   var view = F2(function (action,model) {
      var _p4 = model;
      switch (_p4.ctor)
      {case "FountainModel": return A2($Fountain.view,A2($Signal.forwardTo,action,FountainAction),_p4._0);
         case "PathfinderModel": return A2($Pathfinder.view,A2($Signal.forwardTo,action,PathfinderAction),_p4._0);
         default: return A2($Randomize.view,A2($Signal.forwardTo,action,RandomizeAction),_p4._0);}
   });
   var sliceShow = $SliceShow.show(A2($SliceShow.setView,
   view,
   A2($SliceShow.setUpdate,update,A2($SliceShow.setHash,locationHash,A2($SliceShow.setDimensions,windowDimensions,$SliceShow.init($Slides.slides))))));
   var main = sliceShow.html;
   var tasks = Elm.Native.Task.make(_elm).performSignal("tasks",sliceShow.tasks);
   return _elm.Main.values = {_op: _op
                             ,FountainAction: FountainAction
